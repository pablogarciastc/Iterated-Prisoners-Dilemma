...
public class APavlov_16 extends Agent {
	...
	private String name;
	private String OpHistory = ""; //keep track of the opponent moves
	...
	protected void setup() {
		...
		System.out.println("AdaptivePavlov " + name + " is ready.");
	...
	protected void takeDown() {
		...
		System.out.println("AdaptivePavlov " + name + " terminating.");
	}
	...
	private class Play extends CyclicBehaviour {
			...
			if (msg != null) {
				...
				case s3AwaitingResult:
					...
					if (msg.getPerformative() == ACLMessage.INFORM && msg.getContent().startsWith("Results#")) {
						...
						OpHistory += OpLastMove; // This string is used to know what the op did in the first 6 rounds
		...
		public String choose() {
			...
			if (OpHistory.length() < 6) {// the first time cooperates, then does what the opponent did in the last round, as TFT
				if (OpLastMove.equals("D")) {
					choice = "Action#D";
				} else {
					choice = "Action#C";
				} 
			} else if (OpHistory.length() == 6) { //At the 6th round the agent values what strategy is following the opponent
				if (OpHistory.equals("CCCCCC")) { //If always cooperates it is a cooperative one
					OpClass = "Cooperative";
				} else if (OpHistory.equals("DDDDDD")) { //If always defeats is a ALLD
					OpClass = "ALLD";
				} else if (OpHistory.equals("DCDCDC")) { //This is how a STFT will behave in the first 6 rounds
					OpClass = "STFT";
				} else if (OpHistory.equals("DDCDDC")) { //this is how a Pavlov will behave in the first 6 rounds
					OpClass = "PavlovD";
				} else //if it isnt one of the previous agents, it should be a random one
					OpClass = "Random";

			}

			if (OpHistory.length() >= 6) {
				switch (OpClass) {
				//here the best action against each agent is taken
				case "Random":
					choice = "Action#D"; //with random ones always defeat
					break;
				case "ALLD":
					choice = "Action#D"; //with ALLD always defeat
					break;
				case "STFT":
					if (OpHistory.length() == 6 || OpHistory.length() == 7) {
						choice = "Action#C";
					} else if (OpLastMove.equals("D")) {
						choice = "Action#D";
					} //with STFT defeat if the op defeats, except the 6th and 7th times
					break;
				case "PavlovD":
					choice = "Action#D"; //with Pavlov always defeat
					break;
				case "Cooperative":
					if (OpLastMove.equals("D")) {
						choice = "Action#D"; //with cooperative ones always Cooperate, except if they defeat
					}
					break;
				default:
					choice = "Action#D"; //Theorically never is gonna reach this point
					break;
