...
public class TFT_agent extends Agent {
	...
	protected void setup() {
		...
		System.out.println("TFT " + name + " is ready.");
	...
	protected void takeDown() {
		...
		System.out.println("TFT " + name + " terminating.");
	}
	...
	private class Play extends CyclicBehaviour {
		...
		public void action() {
			...
			if (msg != null) {
				System.out.println(
						name + " received " + msg.getContent() + " from " + msg.getSender().getName()); // DELETEME
				// -------- Agent logic
				switch (state) {
				case s0NoConfig:
				...
				case s2Round:
					...
					if (msg.getPerformative() == ACLMessage.REQUEST /* && msg.getContent().startsWith("Position") */) {
						...
						} //the first time cooperates, then does what the opponent did in the last round
						...
						System.out.println(
								name + ":" + state.name() + " - Unexpected message:" + msg.getContent());
					}
					break;
